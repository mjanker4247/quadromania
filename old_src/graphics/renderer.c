...
/* Optimization: Unified drawing utilities to reduce redundant SDL calls */
static void Graphics_RenderRect(const SDL_Rect* rect, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
    if (rect) {
...
static void Graphics_RenderMessageBox(const char* message, Uint8 r, Uint8 g, Uint8 b)
{
    ...
    base.h = Font_GetHeight() + factor * 2;
    Graphics_RenderRect(&base, 0, 0, 0, 255);

    /* Draw colored message rectangle */
    dest.x = base.x + factor;
    dest.y = base.y + factor;
    dest.w = base.w - 2 * factor;
    dest.h = base.h - 2 * factor;
    Graphics_RenderRect(&dest, r, g, b, 255);

    /* Draw centered text */
    Font_DrawCenteredText(renderer, dest.y + factor, message);
}
...
void Graphics_DrawTitle()
{
	...
	dest.y = Graphics_GetDotHeight() + title_height; /* Position below title with spacing */
...
void Graphics_DrawInstructions()
{
	...
	dest.h = src.h;
	SDL_RenderCopy(renderer, title, &src, &dest);
	Font_DrawCenteredText(renderer, instruction_y, "Instructions");
	/* draw instructions */
	src.x = 0;
	src.y = 0;
	...
	Graphics_DrawText((SCREEN_WIDTH - Font_GetTextWidth((char *)continue_msg)),(SCREEN_HEIGHT - Font_GetHeight()),(char *)continue_msg);
...
void Graphics_ListHighscores(Uint16 nr_of_table)
{
	...
	Font_DrawCenteredText(renderer, highscore_y, txt);
	...
	for(i = 0; i < HIGHSCORE_NR_OF_ENTRIES_PER_TABLE; i++)
	{
		...
		y = highscore_y + ((i + 2) * Font_GetHeight());
...
/**
 * This function draws the standard UI background elements.
 * Combines background, outer frame, and title drawing.
 */
void Graphics_DrawStandardUI()
{
...
/**
 * This function draws text at the given screen coordinate with custom color.
 */
void Graphics_DrawTextWithColor(Uint16 x, Uint16 y, char *text, SDL_Color color)
{
	Font_DrawTextWithColor(renderer, x, y, text, color);
}
...
{
    Graphics_RenderMessageBox("Congratulations! You've won!", 0, 0, 200);
    Graphics_UpdateScreen();
}
...
{
    Graphics_RenderMessageBox("GAME OVER! You hit the turn limit!", 200, 0, 0);
    Graphics_UpdateScreen();
}
...
bool Graphics_Init(bool set_fullscreen)
{
	...
	DEBUG_PRINT("Renderer logical size set to: 1280x960");
	...
	DEBUG_PRINT("Font system initialized");
