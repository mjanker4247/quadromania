...
void InitFont2(SFont_FontInfo *Font)
{
	...
	SDL_SetColorKey(Font->Surface, SDL_SRCCOLORKEY, GetPixel(Font->Surface, 0,
			Font->Surface->h - 1));
}
...
void XCenteredString2(SDL_Surface *Surface, SFont_FontInfo *Font, int y,
		char *text)
{
	PutString2(Surface, Font, Surface->w / 2 - TextWidth2(Font, text) / 2, y,
			text);
}
...
void SFont_InternalInput(SDL_Surface *Dest, SFont_FontInfo *Font, int x, int y,
		int PixelWidth, char *text)
{
	...
	Back = SDL_AllocSurface(Dest->flags, Dest->w, Font->h,
	...
	while (ch != SDLK_RETURN)
	{
		...
		if (SDL_GetTicks() > (unsigned long) blinktimer)
		{
			blink = 1 - blink;
